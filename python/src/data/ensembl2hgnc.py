...
hgnc_reference_table = pd.read_csv(
    "././data/inteim/HGNC symbols.txt", sep='\t', header=0)
hgnc_reference_table.drop_duplicates(inplace=True)
huri = pd.read_csv("././data/raw/HuRI.tsv", sep='\t',
                   header=None, names=['ENSG A', 'ENSG B'])

huri_id = set(list(huri['ENSG A'].unique()) + list(huri['ENSG B'].unique()))
for id in huri_id:
    hgnc_id = hgnc_reference_table.loc[hgnc_reference_table[
        'Ensembl ID(supplied by Ensembl)'] == id, 'Approved symbol'].values
    if len(hgnc_id) > 0:
...
ensg_ids_genecards, missed_ensg_genecards = id_conversion.genecards_search(
    huri_missing_ids, 'ensembl')
